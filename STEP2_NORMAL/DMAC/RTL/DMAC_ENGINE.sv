...
//*********  STEP2: Normal encoding   **********
    ...
    AND2 IDLE_TO_RREQ (.A1(start_i), .A2(len_ne_0), .Y(start_i_and_len_ne_0));
    INV  IDLE_STAY (.A(start_i_and_len_ne_0), .Y(n_start_i_and_len_ne_0));
    ...
    AND2 WDATA_TO_IDLE (.A1(wready_i), .A2(cnt_eq_0), .Y(wready_i_and_cnt_eq_0));
    AND2 WDATA_TO_RREQ (.A1(wready_i), .A2(cnt_ne_0), .Y(wready_i_and_cnt_ne_0));
    
    // 4) Complementary ready/valid signals
    // Used to describe “stay” conditions when a channel is not ready
    ...
    // 5) Inverted bits of the state
    wire n_state_2, n_state_1, n_state_0;
    INV U_NS2 (.A(state[2]), .Y(n_state_2));
    INV U_NS1 (.A(state[1]), .Y(n_state_1));
    INV U_NS0 (.A(state[0]), .Y(n_state_0));

    // ----------------------------------------------
    // 1. Current state decoder
    // Decodes the 3-bit FSM state into individual state signals.
    // Each flag (state_eq_IDLE, state_eq_RREQ, etc.) becomes 1 
    // only when the current state matches its corresponding code.
    // ----------------------------------------------
    wire state_eq_IDLE, state_eq_RREQ, state_eq_RDATA, state_eq_WREQ, state_eq_WDATA;
    
    // 0) IDLE = 3'b000
    NOR3 U_EQ_IDLE (.A1(state[2]), .A2(state[1]), .A3(state[0]), .Y(state_eq_IDLE));
    
    // 1) RREQ = 3'b001 => ~s2 & ~s1 & s0
    wire n_state_2_and_1; 
    AND2 U_RREQ_a (.A1(n_state_2), .A2(n_state_1), .Y(n_state_2_and_1));
    AND2 U_RREQ   (.A1(n_state_2_and_1), .A2(state[0]),  .Y(state_eq_RREQ));
    
    // 2) RDATA = 3'b010 => ~s2 & s1 & ~s0
    wire n_state_2_and_state_1; 
    AND2 U_RDT_a (.A1(n_state_2), .A2(state[1]), .Y(n_state_2_and_state_1));
    AND2 U_RDT   (.A1(n_state_2_and_state_1), .A2(n_state_0), .Y(state_eq_RDATA));
    
    // 3) WREQ = 3'b011 => ~s2 & s1 & s0
    AND2 U_WREQ   (.A1(n_state_2_and_state_1), .A2(state[0]),  .Y(state_eq_WREQ));
    
    // 4) WDATA = 3'b100 => s2 & ~s1 & ~s0
    wire state_2_and_n_state_1; 
    AND2 U_WDT_a (.A1(state[2]), .A2(n_state_1), .Y(state_2_and_n_state_1));
    AND2 U_WDT   (.A1(state_2_and_n_state_1), .A2(n_state_0), .Y(state_eq_WDATA));


    // ----------------------------------------------
    // 2. Next state logic
    // ----------------------------------------------
    // 0) Define when to move to each state
    wire state_n_eq_IDLE, state_n_eq_RREQ, state_n_eq_RDATA, state_n_eq_WREQ, state_n_eq_WDATA;
   
    // 1) state_n_eq_IDLE = (state_eq_IDLE  & n_start_i_and_len_ne_0)
    //                    | (state_eq_WDATA & wready_i_and_cnt_eq_0)
    wire stay_idle, from_wdt_to_idle;
    AND2 U_IDLE_STAY   (.A1(state_eq_IDLE),  .A2(n_start_i_and_len_ne_0), .Y(stay_idle));
    AND2 U_WDT_TO_IDLE (.A1(state_eq_WDATA), .A2(wready_i_and_cnt_eq_0),  .Y(from_wdt_to_idle));
    OR2  U_IDLE_OR0    (.A1(stay_idle),      .A2(from_wdt_to_idle),       .Y(state_n_eq_IDLE));

    // 2) state_n_eq_RREQ = (state_eq_IDLE  & start_i_and_len_ne_0)
    //                    | (state_eq_WDATA & wready_i_and_cnt_ne_0)
    //                    | (state_eq_RREQ  & n_arready)
    wire from_idle_to_rreq, from_wdt_to_rreq, stay_rreq, to_rreq;
    AND2 U_I_TO_RREQ   (.A1(state_eq_IDLE),     .A2(start_i_and_len_ne_0),   .Y(from_idle_to_rreq));
    AND2 U_WDT_TO_RREQ (.A1(state_eq_WDATA),    .A2(wready_i_and_cnt_ne_0),  .Y(from_wdt_to_rreq));
    AND2 U_RREQ_STAY   (.A1(state_eq_RREQ),     .A2(n_arready),              .Y(stay_rreq));
    OR2  U_RREQ_OR0    (.A1(from_idle_to_rreq), .A2(from_wdt_to_rreq),       .Y(to_rreq));
    OR2  U_RREQ_OR1    (.A1(to_rreq),         .A2(stay_rreq),              .Y(state_n_eq_RREQ));

    // 3) state_n_eq_RDATA = (state_eq_RREQ  & arready_i)
    //                     | (state_eq_RDATA & n_rvalid)
    wire from_rreq_to_rdata, stay_rdata;
    AND2 U_RREQ_TO_RDT (.A1(state_eq_RREQ),      .A2(arready_i),       .Y(from_rreq_to_rdata));
    AND2 U_RDT_STAY    (.A1(state_eq_RDATA),     .A2(n_rvalid),        .Y(stay_rdata));
    OR2  U_RDT_OR      (.A1(from_rreq_to_rdata), .A2(stay_rdata),      .Y(state_n_eq_RDATA));

    // 4) state_n_eq_WREQ = (state_eq_RDATA & rvalid_i)
    //                    | (state_eq_WREQ  & n_awready)
    wire from_rdata_to_wreq, stay_wreq;
    AND2 U_RDT_TO_WREQ (.A1(state_eq_RDATA),     .A2(rvalid_i),        .Y(from_rdata_to_wreq));
    AND2 U_WREQ_STAY   (.A1(state_eq_WREQ),      .A2(n_awready),       .Y(stay_wreq));
    OR2  U_WREQ_OR     (.A1(from_rdata_to_wreq), .A2(stay_wreq),       .Y(state_n_eq_WREQ));

    // 5) state_n_eq_WDATA = (state_eq_WREQ  & awready_i)
    //                     | (state[WDATA] & n_wready)
    wire from_wreq_to_wdata, stay_wdata;
    AND2 U_WREQ_TO_WDT (.A1(state_eq_WREQ),      .A2(awready_i),       .Y(from_wreq_to_wdata));
    AND2 U_WDT_STAY    (.A1(state_eq_WDATA),     .A2(n_wready),        .Y(stay_wdata));
    OR2  U_WDT_OR      (.A1(from_wreq_to_wdata), .A2(stay_wdata),      .Y(state_n_eq_WDATA));
    ...
    // ----------------------------------------------
    // 4. Next state encoder (state_n)
    // bit2 is 1, only if state_n_eq_WDATA is 1.
    // bit1 is 1, when state_n_eq_RDATA or state_n_eq_WREQ is 1.
    // bit0 is 1, when state_n_eq_RREQ or state_n_eq_WREQ is 1.
    // For example, if state_n_eq_WREQ = 1, then state_n becomes 3'b011 (WREQ state)
    // since OR2 pairs drive bit1 and bit0 high, while bit2 stays low
    // ----------------------------------------------
    BUF  U_S2N0    (.A(state_n_eq_WDATA), .Y(state_n[2]));
    OR2  U_S1PAIR  (.A1(state_n_eq_RDATA), .A2(state_n_eq_WREQ), .Y(state_n[1]));
    OR2  U_S0PAIR  (.A1(state_n_eq_RREQ),  .A2(state_n_eq_WREQ), .Y(state_n[0]));
    ...
    // ----------------------------------------------
    REG3 U_STATE_REG (.CLK(clk), .RST_n(rst_n), .D(state_n), .Q(state));
    ...
    // 6. Outputs
    // ----------------------------------------------
    BUF U_ARV (.A(state_eq_RREQ),  .Y(arvalid));
    BUF U_RRD (.A(state_eq_RDATA), .Y(rready));
    BUF U_AWV (.A(state_eq_WREQ),  .Y(awvalid));
    BUF U_WV  (.A(state_eq_WDATA), .Y(wvalid));
    BUF U_DN  (.A(state_eq_IDLE),  .Y(done));
